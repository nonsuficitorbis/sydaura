...
export const JoinSession = () => {
  ...
  const handleJoinTrivia = () => {
    ...
    joinSession(sessionData.session.id, sessionData.placement.id, nickname.trim());
  };
  ...
  // 1. CASUAL PLAY HUB OVERLAY ROUTE
  if (viewState === 'CASUAL') {
    return (
      <CasualLobby 
        locationId={sessionData.venue.id} 
        onBack={() => setViewState('HUB')} 
      />
    );
  }

  // 2. LIVE TRIVIA PLAY FLOW
  if (viewState === 'TRIVIA') {
    
    // Sub-view: Joining live trivia session (nickname entry)
    if (gameState === 'JOINING') {
      return (
        <div className="glass-card page-enter" style={{ maxWidth: '500px', margin: '2rem auto' }}>
          <button 
            onClick={() => setViewState('HUB')}
            style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.25rem' }}
          >
            ← Back to Hub
          </button>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <input 
              ...
              placeholder="Enter your trivia nickname..." 
              value={nickname}
    ...
    return (
      <div className="glass-card page-enter" style={{ textAlign: 'center', maxWidth: '500px', margin: '2rem auto' }}>
        ...
        <button className="btn-primary" style={{ marginTop: '1.5rem' }} onClick={() => setViewState('HUB')}>
          Return to Guest Hub
        </button>
      </div>
    );
  ...
  // DEFAULT VIEW: GUEST HUB MENU
  return (
    <GuestHub 
      sessionData={sessionData} 
      onSelectOption={(opt) => setViewState(opt)} 
    />
  );
};
